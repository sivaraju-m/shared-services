...
class EnhancedBigQueryWriter:
    ...
    def __init__(
        self,
        project_id: str,
        dataset_id: str = "ai_trading_backtest",
        ...
        logger.info(
            f"Enhanced BigQuery Writer initialized: {project_id}.{dataset_id}.{table_id}"
        )
    ...
    def _ensure_dataset_exists(self) -> None:
        ...
        try:
            ...
            dataset.location = "US"
            dataset.description = (
                "AI Trading Machine enhanced strategy results with approval workflow"
            )
    ...
    def _ensure_table_exists(self) -> None:
        ...
        try:
            ...
            table.time_partitioning = bigquery.TimePartitioning(
                type_=bigquery.TimePartitioningType.DAY, field="execution_timestamp"
            )
    ...
    def insert_strategy_result(self, result_dict: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if not errors:
                logger.info(
                    f"Successfully inserted strategy result: {row['strategy_name']} - {row['scenario_name']}"
                )
                return {
                    "success": True,
                    "run_id": row["run_id"],
    ...
    def _prepare_enhanced_row_data(self, result_dict: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "execution_timestamp" in result_dict:
            if isinstance(result_dict["execution_timestamp"], str):
                execution_timestamp = datetime.fromisoformat(
                    result_dict["execution_timestamp"].replace("Z", "+00:00")
                )
        ...
        row = {
            ...
            "strategy_category": result_dict.get(
                "strategy_category",
                self._categorize_strategy(result_dict.get("strategy_name", "")),
            ),
            # Scenario Information
            ...
            "scenario_duration_days": self._calculate_duration_days(
            ...
            "downside_volatility": self._safe_float(
                result_dict.get("downside_volatility")
            ),
            ...
            "data_quality_score": self._safe_float(
                result_dict.get("data_quality_score", 1.0)
            ),
            "backtest_start_date": self._parse_date(
                result_dict.get("backtest_start_date")
            ),
            ...
            "approval_date": (
                ...
                if approval_info["approved"] != "PENDING"
                else None
            ),
            # Risk Assessment
            ...
            "liquidity_score": self._safe_float(
                result_dict.get("liquidity_score", 0.8)
            ),
            # Metadata
    ...
    def _calculate_approval_metrics(
        self, result_dict: Dict[str, Any], composite_score: float
    ) -> Dict[str, Any]:
        ...
        approval_score = (
            risk_adjusted_score + consistency_score + drawdown_score + return_score
        ) / 4

        # Determine approval status
        all_criteria_met = (
            meets_sharpe and meets_return and meets_drawdown and meets_win_rate
        )
    ...
    def _categorize_strategy(self, strategy_name: str) -> str:
        ...
        if "momentum" in strategy_name:
            ...
            "lstm" in strategy_name
            or "neural" in strategy_name
            or "ml" in strategy_name
        ):
    ...
    def get_strategy_performance_summary(
        ...
        where_clause = (
            f"WHERE strategy_name = '{strategy_name}'" if strategy_name else ""
        )
