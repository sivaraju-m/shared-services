...
def verify_data_integrity(
    ...
    if row_count < min_rows:
        result["passed"] = False
        result["issues"].append(f"Insufficient data: {row_count} rows (minimum {min_rows})")
    ...
    if check_nulls:
        ...
        if null_count > 0:
            ...
            result["issues"].append(f"Found {null_count} null values in columns: {columns_with_nulls}")
            # Don't fail for nulls, just warn
    
    # Log summary
    if result["passed"]:
        logging.info(f"✅ Data integrity verification passed: {row_count} rows, {len(data.columns)} columns")
    else:
